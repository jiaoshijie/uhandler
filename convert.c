...
ushort* utf32TOutf16(uint ui, size_t* len) {
  if(ui < 0xD800 || (ui > 0xDFFF && ui < 0x10000)) *len = 1;
  else *len = 2;
  ...
  if (*len == 1) {
    *us = (ushort)(ui);
    ...
    ui -= 0x010000;
    us[0] = (ushort)(((0x000ffc00 & ui) >> 10) + 0xD800);  // 0b11111111110000000000 = 0x000ffc00
    us[1] = (ushort)(((0x000003ff & ui) >> 00) + 0xDC00);  // 0b00000000001111111111 = 0x000003ff
  }
  return us;
}
  ...
  *len = 1;
  ...
  if (1 == *len) {
    *uc = (uchar)ui;
    ...
    uc[0] = (uchar)((0x1f >> (*len - 2)) & (ui >> ((*len - 1) * 6))) + utf8_mask[*len - 2];
  }
  return uc;
}

...
uchar* cReadline(FILE* stream) {
  ...
  while (fgets(buf, 4, stream)) {
    ...
    if (line) {
      uchar* temp = line;
      line = (uchar*)malloc(size + buflen + 1);
      memcpy(line, temp, size);
      free(temp);
    } else {
      line = (uchar*)malloc(buflen + 1);
    }
...
uchar* sep_term_input(uchar *line, size_t *pos, int utf8) {
  ...
  size_t len = 1;
  if (utf8) {
    if ((line[*pos] & utf8_mask[0]) == utf8_mask[0]) len++;
    if ((line[*pos] & utf8_mask[1]) == utf8_mask[1]) len++;
    if ((line[*pos] & utf8_mask[2]) == utf8_mask[2]) len++;
  } else {  // gbk(gb2312)
    if (line[*pos] & 0x80) {
      len++;
    }
...
int isUTF8(uchar* line) {
  ...
  while(line[pos]) {
    if ((line[pos] & 0x80) == 0) pos++;
      ...
      pos++;
      while(len--) {
        if ((line[pos++] & 0xC0) != 0x80) return 0;
      }
    }
  }
  return 1;
